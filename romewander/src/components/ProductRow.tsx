        ...
        <section className="py-20 md:py-32 overflow-hidden transition-colors duration-500" style={{ backgroundColor: bgColor }}>
            ...
            <div className="container mx-auto px-6 md:px-16 mb-12">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div className="max-w-2xl">
                        ...
                        >
                            <div className="w-10 h-px bg-card" />
                            <p className="font-inter text-[8px] tracking-[0.4em] uppercase font-bold" style={{ color: '#C9A84C' }}>
                                Essential Rome
                            </p>
                        </div>
                        <h2 
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2 mr-4">
                            {scrollSnaps.map((_, index) => (
                                <div 
                                    key={index} 
                                    className={`h-1 transition-all duration-300 rounded-full ${index === selectedIndex ? 'w-8 bg-card' : 'w-2 bg-card/20'}`} 
                                />
                            ))}
                        </div>
                        <button
                            ...
                            className={`w-12 h-12 rounded-full border flex items-center justify-center transition-all ${!prevBtnEnabled ? 'opacity-30 cursor-not-allowed' : 'hover:bg-card hover:border-border hover:text-white'}`}
                ...
                <div className="embla__container flex gap-6">
                    ...
                    {link && (
                        <div className="embla__slide flex-[0_0_85%] sm:flex-[0_0_45%] lg:flex-[0_0_30%] min-w-0">
                            <Link 
                                href={link}
                                className="h-full flex flex-col items-center justify-center border-2 border-dashed border-border/30 rounded-2xl p-12 group hover:border-border transition-all bg-white/5"
                            >
                                <div className="w-16 h-16 rounded-full bg-card/10 flex items-center justify-center mb-4 group-hover:bg-card group-hover:text-foreground transition-all">
            ...
            {link && (
                <div className="md:hidden text-center mt-12 px-6">
                    <Link
                        href={link}
                        className="inline-flex items-center gap-3 text-[12px] font-bold uppercase tracking-[0.3em] py-4 px-10 rounded-full transition-all active:scale-95"
