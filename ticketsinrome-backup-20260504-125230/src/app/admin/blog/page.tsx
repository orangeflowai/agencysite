...
export default function AdminBlogPage() {
    ...
    const handleAIApply = (content: {
        title: string;
        excerpt: string;
        body: Array<{ _type: string; _key?: string; children?: unknown[]; style?: string }>;
        keywords: string[];
    }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {posts.length > 0 ? (
                    posts.map(post => (
                        <div key={post._id} className="bg-card rounded-xl shadow-sm border border-gray-100 overflow-hidden group hover:shadow-md transition-shadow">
