...
export default function BookingWidget({ tour }: BookingWidgetProps) {
    ...
    const siteId = site?.slug?.current || process.env.NEXT_PUBLIC_SITE_ID || 'ticketsinrome';
    ...
    const Stepper = ({ name, value, min = 0, max = 50 }: { name: string, value: number, min?: number, max?: number }) => (
        <div className="flex items-center gap-3">
            <button
                ...
                disabled={value <= min}
                className="w-10 h-10 rounded-full border border-border flex items-center justify-center text-muted-foreground hover:bg-accent hover:text-primary disabled:opacity-30 transition-all shadow-sm"
            >
                <Minus size={16} />
            </button>
            ...
            <button
                ...
                className="w-10 h-10 rounded-full border border-border flex items-center justify-center text-primary hover:bg-accent disabled:opacity-30 transition-all shadow-sm"
            >
                <Plus size={16} />
            </button>
        </div>
    ...
    return (
        ...
        <div className="space-y-4 font-sans">
            <div className="bg-card rounded-[1.3rem] shadow-[0_20px_50px_rgba(29,161,242,0.08)] border border-border overflow-hidden">

                {/* Header: Title */}
                <div className="px-8 pt-8 pb-4">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-bold  tracking-[0.2em] text-primary bg-primary/10 px-3 py-1 rounded-full">Secure Booking</span>
                    ...
                    <p className="text-xl font-serif font-extrabold text-foreground leading-tight ">Book Your Experience</p>
                </div>
                ...
                <div className="p-8 pt-4 space-y-8">
                    <div>
                        <div className="flex items-center justify-between mb-4">
                            <span className="text-[10px] font-bold  tracking-widest text-muted-foreground">1. Select Date</span>
                    ...
                    {selectedDate && (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pt-2">
                            <div className="flex items-center justify-between mb-4">
                                <span className="text-[10px] font-bold  tracking-widest text-muted-foreground">2. Select Time</span>
                                {selectedTime && <span className="text-xs font-bold text-primary flex items-center gap-1"><CheckCircle size={12} /> {selectedTime}</span>}
                            </div>

                            {loadingAvailability ? (
                                <div className="flex flex-col items-center justify-center py-10 bg-accent/30 rounded-2xl border-2 border-dashed border-border">
                                    <Loader2 className="w-8 h-8 animate-spin text-primary mb-3" />
                                    <span className="text-[10px] text-muted-foreground font-bold  tracking-[0.2em]">Scanning local times...</span>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    {visibleSlots.map(slot => (
                                        <button
                                            ...
                                            className={`
                                                relative py-4 px-2 text-sm font-bold rounded-2xl border transition-all duration-300 text-center flex flex-col items-center justify-center gap-1
                                                ${selectedTime === slot.time
                                                    ? 'bg-primary text-white border-primary shadow-xl shadow-primary/20 scale-[1.05] z-10'
                                                    : 'bg-card text-foreground border-border hover:border-primary/30 hover:bg-accent/30'
                                                }
                ...
                <div className="px-8 py-8 bg-accent/20 border-t border-border">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-2">
                            <Users size={18} className="text-primary" />
                            <span className="text-[10px] font-bold  tracking-widest text-foreground">3. Select Participants</span>
                        </div>
                        <div className="flex items-center gap-1 bg-card/50 px-3 py-1.5 rounded-xl border border-border shadow-sm">
                            <span className="text-[10px] font-bold text-foreground  tracking-widest">{totalGuests} Total</span>
                        </div>
                    </div>

                    <div className="grid gap-4">
                        {currentGuestTypes.map(gt => (
                            <div key={gt.name} className="flex items-center justify-between p-4 bg-card/40 backdrop-blur-sm rounded-2xl border border-border shadow-sm hover:shadow-md transition-all group">
                                <div className="flex flex-col">
                                    ...
                                    {gt.description && <span className="text-[9px] font-bold text-muted-foreground mt-0.5  tracking-[0.1em]">{gt.description}</span>}
                ...
                <div className="px-8 py-4 bg-accent/30 border-t border-border">
                    <div className="flex justify-between items-center text-[9px] font-bold  tracking-[0.2em] text-muted-foreground">
                        <span>Currency</span>
                        <span>Official Price</span>
                    </div>
                    <div className="flex justify-between items-end mt-1">
                        <div className="flex items-center gap-2">
                            <div className="w-6 h-6 bg-card rounded-lg border border-border flex items-center justify-center font-serif font-bold text-[10px] text-foreground shadow-sm ">€</div>
                            <span className="text-[10px] font-bold text-foreground  tracking-widest">Euro (Official)</span>
                        </div>
                        <span className="text-2xl font-serif font-extrabold text-foreground tracking-tighter ">€{totalPrice}</span>
                    </div>
                </div>
                ...
                <div className="p-8 pt-2">
                    ...
                    {validationError && (
                        <div className="mb-4 px-4 py-3 bg-destructive/5 border border-destructive/20 rounded-xl text-[10px] font-bold text-destructive  tracking-widest flex items-center gap-2">
                    ...
                    <button
                        ...
                        className={`w-full py-5 rounded-[1rem] font-bold text-xs  tracking-[0.3em] shadow-2xl transition-all duration-500 transform active:scale-[0.97] flex items-center justify-center gap-3 ${checkingOut || !selectedDate || !selectedTime || totalGuests === 0
                            ? 'bg-muted text-muted-foreground cursor-not-allowed'
                            : 'bg-primary text-white hover:brightness-110 shadow-primary/10 hover:shadow-primary/30'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className="w-full mt-4 py-3 rounded-2xl font-bold text-[10px]  tracking-[0.3em] transition-all duration-300 text-muted-foreground hover:text-primary disabled:opacity-30 flex items-center justify-center gap-2"
                    >
                    ...
                    <div className="mt-6 pt-6 border-t border-border flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="flex gap-0.5">
                                {[1, 2, 3, 4, 5].map(i => <Star key={i} size={10} className="fill-accent text-primary" />)}
                            </div>
                            <span className="text-[9px] font-bold text-foreground  tracking-widest border-l border-border pl-3">4.9 / 5.0 Rating</span>
                        </div>
            ...
            <div className="bg-card rounded-[1.3rem] p-4 border border-border flex items-center gap-4 group cursor-default shadow-sm">
                <div className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center group-hover:scale-110 transition-transform">
                    <CheckCircle size={20} />
                </div>
                <div>
                    <p className="text-[10px] font-bold text-foreground  tracking-[0.15em] leading-none">Free Cancellation</p>
                    <p className="text-[9px] font-medium text-muted-foreground mt-1 tracking-tight ">Full refund up to 24h before</p>
                </div>
            </div>
        </div>
