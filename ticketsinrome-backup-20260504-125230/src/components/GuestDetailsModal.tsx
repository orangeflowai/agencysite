            ...
            {isOpen && (
                ...
                >
                    <motion.div
                        ...
                        className="bg-card w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col overscroll-contain"
                    ...
                    >
                        ...
                        <div
                            className="flex-1 overflow-y-auto bg-muted overscroll-contain scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent"
                            onWheel={(e) => e.stopPropagation()}
                        >
                            <form id="booking-form" onSubmit={handleSubmit} className="p-6 md:p-8 space-y-8">

                                {/* 1. Lead Traveler Contact */}
                                <section className="bg-card p-6 rounded-2xl shadow-sm border border-gray-100">
                                    <h3 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2 pb-2 border-b border-gray-100">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="text" required placeholder="e.g. John" className="w-full p-3 bg-muted border border-border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none"
                                        ...
                                        <div>
                                            ...
                                            <input type="text" required placeholder="e.g. Doe" className="w-full p-3 bg-muted border border-border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none"
                                        ...
                                        <div>
                                            ...
                                            <div className="relative">
                                                <Mail className="absolute left-3 top-3.5 text-muted-foreground w-4 h-4" />
                                                <input type="email" required placeholder="name@example.com" className="w-full p-3 pl-10 bg-muted border border-border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none"
                                ...
                                <section className="bg-card p-6 rounded-2xl shadow-sm border border-gray-100">
                                    ...
                                    <div className="space-y-4">
                                        {guests.map((guest, idx) => (
                                            <div key={guest.index} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end p-4 bg-muted/50 rounded-xl border border-gray-100">
                                                ...
                                                <div className="md:col-span-2">
                                                    <span className={`text-[10px] font-bold  py-1 px-2 rounded-md ${guest.type === 'Adult' ? 'bg-emerald-100 text-emerald-700' :
                                                ...
                                                <div className="md:col-span-9">
                                                    <label className="block text-[10px] font-bold text-muted-foreground  mb-1">Full Name (As on ID)</label>
                                                    <input
                                                        type="text"
                                                        required
                                                        placeholder={`Full Name for ${guest.type}`}
                                                        className="w-full p-2.5 bg-card border border-border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-sm"
                                                        value={guest.name}
                                ...
                                <section className="bg-card p-6 rounded-2xl shadow-sm border border-gray-100">
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-start gap-3">
                                            <input type="checkbox" id="pickup" className="mt-1 w-4 h-4 text-primary rounded focus:ring-emerald-500 border-gray-300"
                                                checked={logistics.pickupRequired} onChange={e => setLogistics({ ...logistics, pickupRequired: e.target.checked })} />
                                            <div>
                                                <label htmlFor="pickup" className="block text-sm font-medium text-foreground font-bold">Pick-up Service Request</label>
                                                <p className="text-xs text-muted-foreground">We'll contact you to arrange pickup from your hotel (Extra fee applies).</p>
                                            </div>
                                        </div>

                                        {logistics.pickupRequired && (
                                            <div className="ml-7">
                                                <label className="block text-xs font-bold text-muted-foreground  mb-1">Hotel/Airbnb Name</label>
                                                <div className="relative">
                                                    <MapPin className="absolute left-3 top-3 text-muted-foreground w-4 h-4" />
                                                    <input type="text" placeholder="Where are you staying?"
                                                        className="w-full p-2.5 pl-9 bg-muted border border-border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-sm"
                        ...
                        {/* Footer */}
                        <div className="p-6 border-t border-gray-100 bg-card flex justify-between items-center shrink-0 z-10">
                            <button onClick={onClose} className="px-6 py-3 font-semibold text-muted-foreground hover:text-foreground transition-colors">
                                Back
                            </button>
                            <button type="submit" form="booking-form" className="px-8 py-4 bg-primary hover:opacity-90 text-white font-bold rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center gap-2">
