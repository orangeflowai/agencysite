 ...
 * INCLUDING downloading images from Sanity and uploading to Payload
 * 
 * Usage:
 *   node migrate-sanity-to-payload-with-images.js [--dry-run] [--site wondersofrome]
 * 
 * Options:
 *   --dry-run    Show what would be migrated without actually doing it
 *   --site       Specify which site to migrate (default: wondersofrome)
 *   --force      Overwrite existing tours in Payload
 *   --skip-images Skip image migration (faster, only migrate data)
 */
...
if (!fs.existsSync(TEMP_DIR) && !isDryRun) {
  fs.mkdirSync(TEMP_DIR, { recursive: true });
}

console.log('\n╔════════════════════════════════════════════════════════════════╗');
console.log('║     SANITY → PAYLOAD MIGRATION TOOL (WITH IMAGES)             ║');
console.log('╚════════════════════════════════════════════════════════════════╝\n');
...
console.log(`Migrate Images: ${skipImages ? 'No (data only)' : 'Yes (full migration)'}\n`);
 ...
 */
async function uploadImageToPayload(imagePath, originalFilename) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to upload image: ${res.status} - ${error}`);
  }

  const data = await res.json();
  return data.doc; // Returns the media document with id, url, filename
}

/**
 * Process tour image: download from Sanity, upload to Payload
 */
async function processImage(sanityImageUrl, tourSlug) {
  if (!sanityImageUrl) return null;

  try {
    // Extract filename from URL or create one
    const urlParts = sanityImageUrl.split('/');
    const sanityFilename = urlParts[urlParts.length - 1];
    const extension = sanityFilename.split('.').pop() || 'jpg';
    const filename = `${tourSlug}-main.${extension}`;

    console.log(`   📥 Downloading image: ${sanityFilename.substring(0, 40)}...`);
    
    // Download image from Sanity
    const localPath = await downloadImage(sanityImageUrl, filename);
    
    console.log(`   📤 Uploading to Payload...`);
    
    // Upload to Payload
    const payloadMedia = await uploadImageToPayload(localPath, filename);
    
    // Clean up local file
    fs.unlinkSync(localPath);
    
    console.log(`   ✅ Image uploaded: ${payloadMedia.id}`);
    
    return payloadMedia.id; // Return the media ID to reference in tour
  } catch (error) {
 ...
 */
function mapSanityToPayload(sanityTour, siteId, payloadMediaId = null) {
  ...
  // Extract image URL (fallback if upload fails)
  let imageUrl = null;
  ...
  // Add image reference if we uploaded it to Payload
  if (payloadMediaId) {
    payloadData.mainImage = payloadMediaId;
  } else if (imageUrl) {
    // Fallback: store Sanity URL
...
async function migrateTours() {
  try {
    ...
    for (let i = 0; i < sanityTours.length; i++) {
      const sanityTour = sanityTours[i];
      const slug = sanityTour.slug?.current || sanityTour.slug;
      const existingTour = payloadTourMap.get(slug);

      console.log(`\n[${i + 1}/${sanityTours.length}] Processing: ${sanityTour.title}`);
      console.log(`   Slug: ${slug}`);

      try {
        let payloadMediaId = null;

        // Process image if not skipping and not dry run
        if (!skipImages && !isDryRun && sanityTour.mainImage?.asset?.url) {
          payloadMediaId = await processImage(sanityTour.mainImage.asset.url, slug);
          if (payloadMediaId) {
            imagesProcessed++;
          } else {
            imagesFailed++;
          }
        } else if (!skipImages && sanityTour.mainImage?.asset?.url) {
          console.log(`   🖼️  Would download and upload image: ${sanityTour.mainImage.asset.url.substring(0, 50)}...`);
        }

        const payloadData = mapSanityToPayload(sanityTour, targetSite, payloadMediaId);
        ...
        if (existingTour) {
          if (forceOverwrite) {
            ...
            console.log(`   ⏭️  Skipping - already exists (use --force to update)`);
            skipped++;
          }
        } else {
        ...
        // Show field summary
        console.log(`   📊 Fields: ${Object.keys(payloadData).length} mapped`);
        if (payloadData.mainImage) console.log(`   🖼️  Image: Uploaded to Payload (ID: ${payloadData.mainImage})`);
        else if (payloadData.imageUrl) console.log(`   🖼️  Image: Sanity URL stored`);
    ...
    console.log(`📊 Total Tours:       ${sanityTours.length}`);
    if (!skipImages) {
      console.log(`\n🖼️  Images Processed:  ${imagesProcessed}`);
