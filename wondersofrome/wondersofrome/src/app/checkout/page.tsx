...
const getStripeKey = (siteId: string) => {
    if (siteId === 'wondersofrome') {
        return process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY_WONDERSOFROME || process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY || '';
    }
    return process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY_ROME_TOUR_TICKETS || process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY || '';
};
...
function CheckoutContent() {
        ...
        try {
            const response = await fetch('/api/create-payment-intent', {
                ...
                body: JSON.stringify({
                    ...
                    mapAddress: bookingData!.tour.mapAddress || '',
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-emerald-50/20 flex items-center justify-center">
                <div className="text-center">
                    <div className="w-12 h-12 border-3 border-emerald-500 border-t-transparent rounded-full animate-spin mx-auto mb-4 border-[3px]"></div>
    ...
    if (error || !bookingData) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-emerald-50/20 flex items-center justify-center p-4">
                <div className="bg-card rounded-2xl shadow-xl border border-slate-100 p-8 max-w-md w-full text-center">
                    <div className="w-16 h-16 rounded-2xl bg-amber-50 flex items-center justify-center mx-auto mb-4">
                        <AlertTriangle className="w-8 h-8 text-amber-500" />
                    </div>
                    <h2 className="text-xl font-bold text-slate-900 mb-2">Something went wrong</h2>
                    <p className="text-slate-500 mb-6 text-sm">{error || 'Booking data not found'}</p>
                    <Link href="/" className="inline-flex items-center gap-2 text-primary font-semibold hover:text-emerald-700 transition-colors text-sm">
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/40 to-emerald-50/30 checkout-scope">
            {/* Header */}
            <header className="bg-card/90 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-50 shadow-sm">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    <Link href="/" className="flex items-center gap-2">
                        {site?.logo ? (
                            <Image src={urlFor(site.logo).url()} alt={site.title} width={120} height={40} className="h-8 w-auto" />
                        ) : (
                            <span className="text-lg font-bold text-slate-900">{site?.title || 'Wonders of Rome'}</span>
                        )}
                    </Link>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        {/* Progress Steps */}
                        <div className="bg-card rounded-2xl p-5 shadow-sm border border-slate-100 overflow-hidden relative">
                            <div className="absolute inset-0 bg-gradient-to-r from-emerald-50/50 via-transparent to-blue-50/50" />
                        ...
                        {step === 1 && (
                            <div className="bg-card rounded-2xl p-6 shadow-sm border border-slate-100 overflow-hidden relative">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input
                                            type="text"
                                            required
                                            className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-emerald-400 outline-none text-sm transition-all bg-slate-50 focus:bg-card"
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                type="email"
                                                required
                                                className="w-full pl-10 pr-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-emerald-400 outline-none text-sm transition-all bg-slate-50 focus:bg-card"
                                    ...
                                    <div className="md:col-span-2">
                                        <PhoneInput
                                            ...
                                            className="w-full"
                                        />
                                        <p className="text-xs text-slate-400 mt-1">For urgent tour updates or emergencies</p>
                                    </div>
                                </div>
                                ...
                                <div className="mt-5 p-4 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl border border-amber-100">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-amber-700 mt-0.5">Names must match your ID/Passport exactly. Security checks are strict at Vatican and Colosseum.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="space-y-3">
                                    {guests.map((guest, idx) => (
                                        <div key={idx} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                                            <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-indigo-500 text-white rounded-xl flex items-center justify-center font-bold text-xs shadow-sm shrink-0">
                                                {idx + 1}
                                            </div>
                                            <input
                                                type="text"
                                                required
                                                className="flex-1 px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400 outline-none text-sm bg-card transition-all"
                                                value={guest.name}
                                                onChange={e => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        className={`p-4 rounded-xl border-2 transition-all ${isSelected
                                                            ? 'border-emerald-500 bg-secondary'
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            ...
                                                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${isSelected ? 'border-emerald-500 bg-primary' : 'border-gray-300'
                                                                }`}>
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex items-start gap-3">
                                            <input
                                                type="checkbox"
                                                id="emailOptIn"
                                                className="mt-1 w-5 h-5 text-primary rounded focus:ring-emerald-500"
                                        ...
                                        <div>
                                            ...
                                            <textarea
                                                rows={3}
                                                className="w-full px-4 py-3 border border-border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none"
                                        ...
                                        <div className="mt-2 space-y-1">
                                            ...
                                            {bookingData.tour.meetingPoint && (
                                                <div className="flex flex-col gap-1 pt-1 mt-1 border-t border-border">
                                                    ...
                                                    <a
                                                        href={bookingData.tour.mapAddress?.startsWith('http') ? bookingData.tour.mapAddress 
                                                            : bookingData.tour.location?.startsWith('http') ? bookingData.tour.location 
                                                            : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent((bookingData.tour.mapAddress || bookingData.tour.location || bookingData.tour.meetingPoint?.split(/(?:\n|\. )/)[0] || '') + ' Rome')}`
                                                        }
                                                        target="_blank"
            ...
            <div className="p-4 bg-muted border border-border rounded-xl">
                <div className="flex items-start gap-3">
                    <input
                        type="checkbox"
                        id="policyAcceptance"
                        className="mt-1 w-4 h-4 text-primary border-gray-300 rounded focus:ring-emerald-500 cursor-pointer"
