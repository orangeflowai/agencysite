...
function SuccessContent() {
    ...
    async function fetchTourDetails(slug: string) {
        try {
            // Fetch real meeting point from Sanity by tour slug
    ...
    const addToAppleCalendar = () => {
        ...
        const icsContent = [
            ...
            `DESCRIPTION:Booking ID: ${booking.id?.slice(-8).toUpperCase()}`,
            `LOCATION:${tour?.meetingPoint || 'Rome, Italy'}`,
            'END:VEVENT', 'END:VCALENDAR'
        ].join('\r\n');
        const blob = new Blob([icsContent], { type: 'text/calendar' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a'); a.href = url; a.download = `rome-tour-wonders.ics`; a.click();
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            ...
                            <button
                                onClick={() => {
                                    if (navigator.share) {
                                        navigator.share({
                                            title: 'My Wonders of Rome Booking',
                    ...
                    {booking && (
                        <div className="mt-5 pt-5 border-t border-gray-100">
                            ...
                            <div className="flex flex-wrap gap-3">
                                ...
                                >
                                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none"><rect width="24" height="24" rx="3" fill="#4285F4"/><path d="M12 10.5V7H9v7h6v-3.5h-3z" fill="white"/></svg>
                                    Google Calendar
                                </button>
                                <button
