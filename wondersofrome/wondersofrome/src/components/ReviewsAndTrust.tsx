...
const trustItems = [
  { icon: Shield,      title: 'Secure Booking',    sub: 'SSL Encrypted',       bg: 'bg-secondary',     text: 'text-sky-600' },
  ...
  { icon: Clock,       title: 'Free Cancellation', sub: 'Up to 24h Before',    bg: 'bg-secondary', text: 'text-primary' },
    ...
    <section className="bg-card py-24 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-foreground tracking-tight leading-tight mb-3">
          ...
          <div className="flex items-center gap-5 mt-5 flex-wrap justify-center">
            {[
              ...
              <div key={p.label} className="flex items-center gap-1.5 text-sm text-muted-foreground">
                <span className={`${p.color} font-bold text-base leading-none`}>{p.letter}</span>
                <span>{p.label}</span>
        ...
        <div className="mt-16 pt-12 border-t border-border">
          <p className="text-center text-xs font-bold  tracking-widest text-muted-foreground mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 max-w-4xl mx-auto">
            {trustItems.map((item) => {
              ...
              return (
                <div
                  key={item.title}
                  className={`${item.bg} rounded-2xl p-4 flex flex-col items-center text-center gap-2 border border-border`}
                >
