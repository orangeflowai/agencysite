...
export default function MobileStickyBar({ price }: { price: number }) {
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 p-4 bg-card/80 backdrop-blur-xl border-t border-border flex items-center justify-between shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
      <div>
        <p className="text-[10px] font-bold text-muted-foreground  tracking-widest">From</p>
      ...
      <button
        ...
        className="bg-accent text-primary px-8 py-3 rounded-2xl font-bold text-xs  tracking-[0.2em] shadow-xl active:scale-95 transition-all"
      >
        Check Availability
      </button>
    </div>
