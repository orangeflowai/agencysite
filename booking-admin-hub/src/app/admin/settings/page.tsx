...
export default function SettingsPage() {
    const { currentSite, sites, selectedSiteId } = useAdmin();
            ...
            const res = await fetch('/api/v1/settings', {
                ...
                body: JSON.stringify({
                    ...
                    business_info: {
                        ...
                        vatNumber: businessForm.vatNumber,
                        reaNumber: businessForm.reaNumber,
                        registeredAddress: businessForm.registeredAddress,
                        pecEmail: businessForm.pecEmail,
                        sdiCode: businessForm.sdiCode,
                        shareCapital: businessForm.shareCapital,
                    },
                    contact_info: {
                        contactEmail: contactForm.contactEmail,
                        contactPhone: contactForm.contactPhone,
                        whatsappNumber: contactForm.whatsappNumber,
                        officeAddress: contactForm.officeAddress,
                    },
                    gdpr_settings: gdprForm,
                }),
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || 'Save failed');
            setToast({ message: 'Settings saved to Supabase + Sanity CMS!', type: 'success' });
    ...
    const Toast = toast ? (
        <div className={`fixed bottom-6 right-6 z-[100] flex items-center gap-3 px-5 py-4 rounded-xl shadow-2xl text-white text-sm font-medium ${
            ...
            {toast.type === 'success' ? <Check className="w-4 h-4" /> : <span>✕</span>}
            {toast.message}
        </div>
    ) : null;
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    placeholder="e.g., Tickets in Rome S.r.l."
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        placeholder="3517869798 (digits only)"
                                        className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all"
                                    />
                                </div>
