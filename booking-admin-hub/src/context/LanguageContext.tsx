...
const translations = {
    ...
    pt: {
        ...
        'cat.hidden_subtitle': 'Catacumbas, passeios de Golf Cart, passeios gastronomicos.',
        'cat.view_all': 'Ver Todos',
...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
        ...
        if (typeof window !== 'undefined') {
            ...
            setTimeout(() => {
                ...
                if (googleTranslateDropdown) {
                    ...
                    // If no exact match (like 'en'), try to find it in options
                    const options = Array.from(googleTranslateDropdown.options);
                    const hasExactMatch = options.some(opt => opt.value === lang);

                    if (!hasExactMatch && lang !== 'en') {
                        // Try matching start (e.g. 'pt' matching 'pt-PT')
                        const bestMatch = options.find(opt => opt.value.startsWith(lang));
                        if (bestMatch) {
                            targetValue = bestMatch.value;
                        }
                    } else if (lang === 'en') {
                        // English is usually empty or 'en'
                        const enOption = options.find(opt => opt.value === 'en' || opt.value === '');
                        targetValue = enOption ? enOption.value : '';
                    }

                    googleTranslateDropdown.value = targetValue;
    ...
    useEffect(() => {
        setIsHydrated(true);
        const saved = localStorage.getItem('rome-tour-tickets-lang') as Language;
    ...
    const setLanguage = useCallback((lang: Language) => {
        ...
        localStorage.setItem('rome-tour-tickets-lang', lang);
    ...
    const currentTranslations = useMemo(() => translations[language] as Record<string, string>, [language]);
    ...
    const contextValue = useMemo(() => ({ language, setLanguage, t, translateTour }), [language, setLanguage, t, translateTour]);
