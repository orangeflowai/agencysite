...
function SuccessContent() {
    ...
    const addToGoogleCalendar = () => {
        if (!booking) return;
        const startDate = booking.date.replace(/-/g, '') + 'T' + (booking.time?.replace(':', '') || '0900') + '00';
        const endDate = booking.date.replace(/-/g, '') + 'T' + String(parseInt((booking.time?.replace(':', '') || '0900').slice(0, 2)) + 3).padStart(2, '0') + '0000';
        const params = new URLSearchParams({
            action: 'TEMPLATE',
            text: `Rome Tour: ${booking.tour_title}`,
            dates: `${startDate}/${endDate}`,
            details: `Booking Ref: ${booking.booking_ref}\nMeeting point: ${booking.meeting_point || 'See confirmation email'}`,
            location: booking.meeting_point || 'Rome, Italy',
        });
    ...
    const addToAppleCalendar = () => {
        ...
        const icsContent = [
            ...
            'BEGIN:VEVENT',
            `SUMMARY:Rome Tour: ${booking.tour_title}`,
            `DTSTART:${booking.date.replace(/-/g, '')}T${(booking.time?.replace(':', '') || '0900')}00`,
            `DTEND:${booking.date.replace(/-/g, '')}T${String(parseInt((booking.time?.replace(':', '') || '1200').slice(0, 2)) + 3).padStart(2, '0')}0000`,
        ...
        const a = document.createElement('a'); a.href = url; a.download = `rome-tour-${booking.booking_ref}.ics`; a.click();
                                ...
                                <div className="mt-6 pt-5 border-t border-gray-100">
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        <button
                                            ...
                                            className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:border-emerald-500 hover:bg-emerald-50 transition-colors"
                                        >
                                            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none"><rect width="24" height="24" rx="3" fill="#4285F4"/><path d="M12 10.5V7H9v7h6v-3.5h-3z" fill="white"/><path d="M17 14h-5v3h5z" fill="white" opacity=".7"/></svg>
                                            Google Calendar
                                        </button>
                                        <button
                                            onClick={addToAppleCalendar}
                                            className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:border-gray-400 hover:bg-gray-50 transition-colors"
                                        >
                                            🍎 Apple / Outlook (.ics)
                                        </button>
                                    </div>
