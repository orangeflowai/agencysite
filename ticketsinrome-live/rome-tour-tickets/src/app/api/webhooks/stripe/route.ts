...
function determineSiteFromEvent(event: any): string {
    ...
    if (session?.metadata?.siteId) {
        return session.metadata.siteId;
    }
    
    // Try to determine from success_url
...
async function sendEmails(siteId: string, email: string, name: string, tourTitle: string, date: string, time: string, guests: string, totalAmount: number, orderId: string, metadata: any) {
    ...
    const senderName = siteId === 'wondersofrome' ? 'Wonders of Rome' : 'Tickets in Rome';
    const senderEmail = siteId === 'wondersofrome' 
        ? 'info@wondersofrome.com' 
        : 'info@ticketsinrome.com'; // Use your configured email
    const adminEmails = siteId === 'wondersofrome'
        ? ['bookings@wondersofrome.com']
        : ['ticketsinrome@gmail.com', 'abiile@ticketsinrome.com'];

    // Generate PIN (last 6 chars of ID + logic) - Mock for now
    const pin = orderId.slice(-6).toUpperCase();

    console.log(`📧 Sending emails for booking ${orderId.slice(-6)} to ${email}`);
    ...
    try {
        ...
        const adminResult = await resend.emails.send({
            from: `${senderName} Alerts <info@${siteId === 'wondersofrome' ? 'wondersofrome.com' : 'ticketsinrome.com'}>`,
            to: adminEmails,
...
export async function POST(request: Request) {
    ...
    if (event.type === 'payment_intent.succeeded') {
        ...
        console.log(`[${siteId}] Use PaymentIntent: ${tourTitle} for ${name}`);
