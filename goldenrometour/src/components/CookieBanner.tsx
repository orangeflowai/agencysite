                ...
                >
                    <div className="max-w-5xl mx-auto bg-cream border border-forest p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center gap-6 shadow-2xl">
                        <div className="flex-1">
                            <h3 className="font-heading text-xl font-bold text-secondary mb-2  underline decoration-primary/20">{bannerTitle}</h3>
                            <p className="font-body text-xs text-secondary leading-relaxed  tracking-tighter opacity-80">
                                {bannerText}
                                {' '}{getComplianceText()}
                                <Link href={privacyPolicyLink} className="text-secondary hover:underline font-bold ml-1">
                                    {privacyPolicyText}
                                </Link>.
                            </p>
                        </div>
                        <div className="flex gap-3 shrink-0 w-full md:w-auto">
                            <button
                                onClick={decline}
                                className="flex-1 md:flex-none px-6 py-3 border border-forest text-secondary font-bold  tracking-tight hover:bg-forest hover:text-cream transition-colors text-[10px]"
                            >
                                {declineButtonText}
                            </button>
                            <button
                                onClick={accept}
                                className="flex-1 md:flex-none px-8 py-3 bg-forest text-cream font-bold  tracking-tight hover:bg-forest-light transition-colors text-[10px] shadow-lg shadow-forest/10"
                            >
                                {acceptButtonText}
                            </button>
                        </div>
