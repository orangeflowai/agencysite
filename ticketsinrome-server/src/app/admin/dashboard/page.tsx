...
export default function DashboardPage() {
    ...
    async function loadDashboard() {
        try {
            ...
            // Calculate stats — handle both 'confirmed' (new) and 'paid' (old) status
            const isPaid = (b: any) => b.status === 'paid' || b.status === 'confirmed';
            const isPending = (b: any) => b.status === 'pending' || b.status === 'pending_payment';
            const isCancelled = (b: any) => b.status === 'cancelled' || b.status === 'refunded';
            const getPrice = (b: any) => Number(b.total_amount ?? b.total_price ?? 0);
            ...
            const totalRevenue = paid.reduce((sum, b) => sum + getPrice(b), 0);
    ...
    return (
        <div className="space-y-8">
            ...
            <div>
                ...
                <p className="text-muted-foreground mt-1">Overview of your tour business performance</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-card rounded-xl shadow-sm border border-border p-6">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground mt-1">€{(stats?.thisWeekRevenue || 0).toFixed(2)} total</p>
                        </div>
                    </div>
                ...
                <div className="bg-card rounded-xl shadow-sm border border-border p-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <QuickAction href="/admin/addons" icon={Store} label="Manage Add-ons" color="blue" />
                        ...
                        {recentBookings.map(booking => (
                            <div key={booking.id} className="px-6 py-4 flex items-center justify-between hover:bg-muted/50 transition-colors">
                                <div className="flex items-center gap-4">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${booking.status === 'paid' ? 'bg-blue-100' :
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-muted-foreground">{booking.lead_first_name ? `${booking.lead_first_name} ${booking.lead_last_name || ''}`.trim() : (booking.customer_name || 'Unknown')} · {booking.date}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="font-bold text-foreground">€{(Number(booking.total_amount ?? booking.total_price ?? 0)).toFixed(2)}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <StatusCard label="Paid" count={stats?.paidBookings || 0} icon={CheckCircle} color="blue" />
...
function QuickAction({ href, icon: Icon, label, color }: { href: string; icon: any; label: string; color: string }) {
    const colors: Record<string, string> = {
        ...
        gray: 'bg-muted text-muted-foreground group-hover:bg-gray-100',
    };

    return (
        <Link href={href} className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted transition-all group">
            ...
            <ArrowUpRight className="w-4 h-4 text-muted-foreground ml-auto group-hover:text-muted-foreground" />
        </Link>
    );
}
...
function StatusCard({ label, count, icon: Icon, color }: { label: string; count: number; icon: any; color: string }) {
    const colors: Record<string, { bg: string; text: string; icon: string }> = {
        blue: { bg: 'bg-secondary', text: 'text-blue-700', icon: 'text-primary' },
