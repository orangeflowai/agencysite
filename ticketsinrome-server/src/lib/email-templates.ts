...
export function generateCustomerEmail(
  ...
  // Branding per site
  const brandConfig: Record<string, { color: string; name: string; domain: string }> = {
    wondersofrome:   { color: '#064034', name: 'Wonders of Rome',  domain: 'wondersofrome.com' },
    ticketsinrome:   { color: '#1e40af', name: 'ticketsinrome', domain: 'ticketsinrome.com' },
    goldenrometour:  { color: '#C9A84C', name: 'Golden Rome Tour',  domain: 'goldenrometour.com' },
    romanvaticantour:{ color: '#8B0000', name: 'Roman Vatican Tour', domain: 'romanvaticantour.com' },
    romewander:      { color: '#064034', name: 'Rome Wander',       domain: 'romewander.com' },
    'rome-tour-tickets': { color: '#8B1A1A', name: 'ticketsinrome', domain: 'ticketsinrome.com' },
  };
  const brand = brandConfig[siteId] || brandConfig['ticketsinrome'];
  ...
  // Default meeting points per site
  const defaultMeetingPoints: Record<string, string> = {
    wondersofrome: 'Via Tunisi 43, 00192 Roma RM, Italy',
    ticketsinrome: 'Via Germanico 8, 00192 Roma RM, Italy',
    'rome-tour-tickets': 'Via Germanico 8, 00192 Roma RM, Italy',
  };
  const meetingPoint = (data.metadata.meetingPoint && data.metadata.meetingPoint !== 'See booking confirmation for details')
    ...
    : (defaultMeetingPoints[siteId] || 'Via Germanico 8, 00192 Roma RM, Italy');
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
  ...
  ${data.metadata?.specialRequests ? `<tr><td style="background:#fff;padding:0 40px 24px;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#fffbeb;border-left:4px solid #f59e0b;border-radius:0 12px 12px 0;">
      <tr><td style="padding:14px 16px;">
        <p style="margin:0 0 6px;font-size:11px;font-weight:800;letter-spacing:1px;text-transform:uppercase;color:#92400e;">&#128221; Special Requests</p>
        <p style="margin:0;font-size:13px;color:#78350f;line-height:1.5;">${data.metadata.specialRequests}</p>
      </td></tr>
    </table>
  </td></tr>` : ''}
  <tr><td style="background:#fff;padding:0 40px 24px;">
...
export function generateAdminEmail(
  ...
  const brandNames: Record<string, string> = { wondersofrome: 'Wonders of Rome', ticketsinrome: 'ticketsinrome', 'rome-tour-tickets': 'ticketsinrome', goldenrometour: 'Golden Rome Tour', romanvaticantour: 'Roman Vatican Tour', romewander: 'Rome Wander' };
  const brandName = brandNames[siteId] || 'ticketsinrome';
  const bookingRef   = data.orderId.slice(-8).toUpperCase();
...
<div class="wrap">
  ...
  <div class="section">
    ...
    <table>
      ...
      <tr><td>Notes</td><td>${data.metadata?.specialRequests ? `<strong style="color:#92400e;">${data.metadata.specialRequests}</strong>` : '<span style="color:#9ca3af;">None</span>'}</td></tr>
    </table>
  </div>
