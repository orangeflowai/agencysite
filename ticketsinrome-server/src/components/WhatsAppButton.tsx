...
export default function WhatsAppButton() {
  const site = useSite();
  const phone   = site?.whatsappNumber || process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '';
  const message = encodeURIComponent("Hi! I'm interested in booking a Rome tour.");
  ...
  return (
    ...
    >
      ...
      {/* Button — collapses to icon-only after 400px scroll */}
      <div className={`relative bg-[#25D366] rounded-2xl flex items-center justify-center shadow-xl shadow-green-500/30 hover:scale-110 active:scale-95 transition-all duration-300 ${
        scrolled ? 'w-11 h-11' : 'w-12 h-12'
      }`}>
        <span className="absolute inset-0 rounded-2xl bg-[#25D366] animate-ping opacity-20" />
        <svg viewBox="0 0 24 24" fill="white" className={`relative z-10 transition-all duration-300 ${scrolled ? 'w-5 h-5' : 'w-6 h-6'}`}>
