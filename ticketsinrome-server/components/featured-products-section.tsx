      ...
      <div className="grid grid-cols-1 gap-6 px-6 pb-20 md:grid-cols-3 md:px-12 lg:px-20">
        {featuredTours.map((tour) => (
          <Link 
            ...
            className="group cursor-pointer flex flex-col bg-card rounded-2xl border border-border overflow-hidden hover:shadow-lg transition-shadow duration-300"
          >
            {/* Image */}
            <div className="relative aspect-[4/3] overflow-hidden shrink-0">
              ...
              {tour.category && (
                <div className="absolute top-4 left-4">
                  <span className="bg-foreground text-background px-3 py-1 text-xs font-medium rounded-full capitalize">
                    {tour.category}
                  </span>
                </div>
              )}
            ...
            {/* Content — flex-1 keeps all cards equal height */}
            <div className="p-6 flex flex-col flex-1">
              ...
              <h3 className="text-foreground text-base font-semibold leading-snug flex-1">
                {tour.title}
              </h3>
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-border">
                <span className="text-sm text-muted-foreground">{tour.duration}</span>
                <span className="text-lg font-semibold">From €{tour.price}</span>
              </div>
            </div>
          </Link>
        ))}
