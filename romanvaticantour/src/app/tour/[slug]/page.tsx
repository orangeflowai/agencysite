...
function TrustBadge({ icon, label, sub }: { icon: React.ReactNode; label: string; sub: string }) {
    return (
        <div className="flex items-center gap-3 px-4 py-3 bg-card rounded-xl border border-[#b19681]/20 shadow-sm">
            ...
            <div>
                <p className="text-[10px] font-sans font-bold  tracking-widest text-[#5c4b3e] leading-tight">{label}</p>
                <p className="text-[9px] text-[#85766a] leading-tight mt-0.5  tracking-tight">{sub}</p>
            </div>
        </div>
    );
}
                ...
                <div className="absolute bottom-0 left-0 right-0 z-10 pointer-events-none">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 md:pb-12">
                        <div className="max-w-3xl pointer-events-auto space-y-3">
                            <Link href="/" className="inline-flex items-center gap-1.5 text-white/70 hover:text-white text-[10px] font-sans font-bold  tracking-widest transition-colors mb-2">
                                <ArrowLeft className="w-3 h-3" /> Back to Tours
                            </Link>
                            <span className="inline-block bg-primary text-white text-[10px] font-sans font-bold  tracking-[0.2em] px-3 py-1 rounded-full shadow-lg">
                                {tour.category}
                            </span>
                            <h1 className="text-3xl md:text-4xl lg:text-5xl font-serif font-bold text-white drop-shadow-lg leading-tight ">
                                {tour.title}
                            </h1>
                            <div className="flex flex-wrap items-center gap-4 text-white/90 text-[10px] font-sans font-bold  tracking-widest">
                    ...
                    <div className="lg:col-span-2 space-y-10">
                        ...
                        <section>
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <h2 className="text-2xl font-serif font-bold text-[#5c4b3e] ">Tour Overview</h2>
                                <div className="h-px flex-1 bg-[#b19681]/30"></div>
                            </div>
                                    ...
                                    <PortableText
                                        ...
                                        components={{
                                            ...
                                            block: {
                                                ...
                                                h2: ({ children }) => <h2 className="text-xl font-serif font-bold mt-6 mb-3 text-[#5c4b3e] ">{children}</h2>,
                                                h3: ({ children }) => <h3 className="text-lg font-serif font-semibold mt-5 mb-2 text-[#5c4b3e] ">{children}</h3>,
                                            },
                                            marks: {
                        ...
                        {tour.highlights && tour.highlights.length > 0 && (
                            <section>
                                <h2 className="text-2xl font-serif font-bold text-[#5c4b3e] mb-6 ">Highlights</h2>
                        ...
                        {tour.itinerary && tour.itinerary.length > 0 && (
                            <section>
                                <h2 className="text-2xl font-serif font-bold text-[#5c4b3e] mb-6 ">Itinerary</h2>
                                <div className="relative pl-6 border-l border-[#b19681]/30 space-y-8">
                                    {tour.itinerary.map((stop, index) => (
                                        <div key={index} className="relative">
                                            ...
                                            <p className="text-[10px] font-sans font-bold  tracking-[0.2em] text-primary mb-1">Stop {index + 1}</p>
                                            <h3 className="text-lg font-serif font-bold text-[#5c4b3e] mb-1 ">{stop.title}</h3>
                                            {stop.duration && <p className="text-[10px] font-sans font-bold text-[#85766a] mb-2  tracking-widest">{stop.duration}</p>}
                            ...
                            <section className="grid sm:grid-cols-2 gap-8">
                                {tour.includes && tour.includes.length > 0 && (
                                    <div className="bg-card rounded-2xl border border-[#b19681]/20 p-6 shadow-sm">
                                        <h3 className="text-lg font-serif font-bold text-[#5c4b3e] mb-4 flex items-center gap-2 ">
                                ...
                                {tour.excludes && tour.excludes.length > 0 && (
                                    <div className="bg-[#e7dbbf]/20 rounded-2xl border border-[#b19681]/10 p-6 shadow-sm">
                                        <h3 className="text-lg font-serif font-bold text-[#5c4b3e] mb-4 flex items-center gap-2  opacity-80">
                        ...
                        {(tour.meetingPoint || (tour.importantInfo && tour.importantInfo.length > 0)) && (
                            <section className="bg-[#e7dbbf]/40 rounded-2xl border border-[#b19681]/20 p-8 space-y-6 shadow-sm">
                                {tour.meetingPoint && (
                                    <div>
                                        <h3 className="text-lg font-serif font-bold text-[#5c4b3e] mb-3 flex items-center gap-2 ">
                                        ...
                                        <a
                                            ...
                                            className="inline-flex items-center gap-2 text-[10px] font-sans font-bold text-primary  tracking-[0.2em] hover:opacity-80 transition-opacity"
                                        >
