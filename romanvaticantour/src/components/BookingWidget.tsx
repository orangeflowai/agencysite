...
export default function BookingWidget({ tour }: BookingWidgetProps) {
    ...
    const Stepper = ({ name, value, min = 0, max = 50 }: { name: string, value: number, min?: number, max?: number }) => (
        <div className="flex items-center gap-3">
            <button
                ...
                disabled={value <= min}
                className="w-10 h-10 rounded-xl border border-border/20 flex items-center justify-center text-muted-foreground hover:bg-muted hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed transition-all active:scale-95 shadow-sm"
            >
                <Minus size={16} />
            </button>
            <span className="w-6 text-center font-serif font-bold text-foreground text-lg ">{value}</span>
            <button
                onClick={() => {
                ...
                className="w-10 h-10 rounded-xl border border-border/20 flex items-center justify-center text-primary hover:bg-muted disabled:opacity-30 disabled:cursor-not-allowed transition-all active:scale-95 shadow-sm"
            >
                <Plus size={16} />
            </button>
        </div>
    ...
    return (
        ...
        <div className="space-y-4 widget-scope">
            <div className="bg-card rounded-3xl shadow-[0_20px_50px_rgba(141,157,79,0.08)] border border-border/20 overflow-hidden font-sans">

                {/* Header: Title */}
                <div className="px-8 pt-8 pb-4">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-[11px] font-bold tracking-[0.15em] text-primary bg-primary/10 px-3 py-1 rounded-full">Secure Booking</span>
                    ...
                    <p className="text-xl font-serif font-bold text-foreground leading-tight ">Book Your Experience</p>
                </div>
                ...
                <div className="p-8 pt-4 space-y-8">
                    <div>
                        <div className="flex items-center justify-between mb-4">
                            <span className="text-[11px] font-bold  tracking-widest text-muted-foreground">1. Select Date</span>
                    ...
                    {selectedDate && (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pt-2">
                            <div className="flex items-center justify-between mb-4">
                                <span className="text-[11px] font-bold  tracking-widest text-muted-foreground">2. Select Time</span>
                                {selectedTime && <span className="text-xs font-bold text-primary flex items-center gap-1"><CheckCircle size={12} /> {selectedTime}</span>}
                            </div>

                            {loadingAvailability ? (
                                <div className="flex flex-col items-center justify-center py-10 bg-muted/30 rounded-2xl border-2 border-dashed border-border/20">
                                    <Loader2 className="w-8 h-8 animate-spin text-primary mb-3" />
                                    <span className="text-[8px] text-muted-foreground font-bold  tracking-[0.2em]">Scanning local times...</span>
                                </div>
                                    ...
                                    {visibleSlots.map(slot => (
                                        <button
                                            ...
                                            className={`
                                                ...
                                                ${selectedTime === slot.time
                                                    ...
                                                    : 'bg-card/50 text-foreground border-border/20 hover:border-primary/30 hover:bg-card/80'
                                                }
                                        ...
                                        >
                                            ...
                                            {slot.available_slots < 5 && slot.available_slots > 0 &&
                                                <span className={`text-[11px] font-bold tracking-tight ${selectedTime === slot.time ? 'text-white/80' : 'text-accent'}`}>
                                ...
                                <div className="text-sm text-destructive font-bold py-6 px-4 bg-destructive/5 rounded-2xl border-2 border-destructive/10 flex flex-col items-center justify-center gap-3">
                                    <div className="w-12 h-12 bg-destructive/10 rounded-full flex items-center justify-center">
                                        <AlertTriangle size={24} className="text-destructive" />
                                    </div>
                                    <span className=" tracking-widest text-[8px] font-sans">Fully booked for this date</span>
                                </div>
                            )}
                        </div>
                    )}
                ...
                <div className="px-8 py-8 bg-muted/30 border-t border-border/20">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-2">
                            <Users size={18} className="text-primary" />
                            <span className="text-[11px] font-bold  tracking-widest text-foreground">3. Select Participants</span>
                        </div>
                        <div className="flex items-center gap-1 bg-card/50 px-3 py-1.5 rounded-xl border border-border/20 shadow-sm">
                            <span className="text-[8px] font-bold text-foreground  tracking-widest">{totalGuests} Total</span>
                        </div>
                    </div>

                    <div className="grid gap-4">
                        {currentGuestTypes.map(gt => (
                            <div key={gt.name} className="flex items-center justify-between p-4 bg-card/40 backdrop-blur-sm rounded-2xl border border-border/20 shadow-sm hover:shadow-md transition-all group">
                                <div className="flex flex-col">
                                    <span className="text-sm font-serif font-bold text-foreground group-hover:text-primary transition-colors  tracking-wide ">{gt.name}</span>
                                    {gt.description && <span className="text-[8px] font-bold text-muted-foreground mt-0.5  tracking-[0.1em]">{gt.description}</span>}
                                    <span className="text-xs font-bold text-primary mt-1 flex items-center gap-1">
                                        €{gt.price} <span className="text-[8px] text-muted-foreground font-bold  tracking-widest">/ Person</span>
                                    </span>
                                </div>
                ...
                <div className="px-8 py-4 bg-muted/50 border-t border-border/20">
                    <div className="flex justify-between items-center text-[11px] font-bold tracking-[0.15em] text-muted-foreground">
                        <span>Currency</span>
                        <span>Official Price</span>
                    </div>
                    <div className="flex justify-between items-end mt-1">
                        <div className="flex items-center gap-2">
                            <div className="w-6 h-6 bg-card rounded-lg border border-border/20 flex items-center justify-center font-serif font-bold text-[8px] text-foreground shadow-sm ">€</div>
                            <span className="text-[8px] font-bold text-foreground  tracking-widest">Euro (Official)</span>
                        </div>
                        <span className="text-2xl font-serif font-bold text-foreground tracking-tighter ">€{totalPrice}</span>
                    </div>
                </div>
                ...
                <div className="p-8 pt-2">
                    ...
                    {validationError && (
                        <div className="mb-4 px-4 py-3 bg-destructive/5 border border-destructive/20 rounded-xl text-[8px] font-bold text-destructive  tracking-widest flex items-center gap-2">
                    ...
                    <button
                        ...
                        className={`w-full py-5 rounded-2xl font-bold text-xs  tracking-[0.3em] shadow-2xl transition-all duration-500 transform active:scale-[0.97] flex items-center justify-center gap-3 ${checkingOut || !selectedDate || !selectedTime || totalGuests === 0
                            ? 'bg-muted text-muted-foreground cursor-not-allowed'
                            : 'bg-primary text-primary-foreground hover:opacity-90 shadow-primary/10 hover:shadow-primary/30'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className="w-full mt-4 py-3 rounded-2xl font-bold text-[8px]  tracking-[0.3em] transition-all duration-300 text-muted-foreground hover:text-primary disabled:opacity-30 flex items-center justify-center gap-2"
                    >
                    ...
                    <div className="mt-6 pt-6 border-t border-border/20 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[8px] font-bold text-foreground  tracking-widest border-l border-border/30 pl-3">4.9 / 5.0 Rating</span>
                            </div>
            ...
            <div className="bg-primary/20 backdrop-blur-sm rounded-2xl p-4 border border-white/20 flex items-center gap-4 group cursor-default shadow-sm">
                <div className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center group-hover:scale-110 transition-transform">
                    <CheckCircle size={20} />
                </div>
                <div>
                    <p className="text-[8px] font-bold text-foreground  tracking-[0.15em] leading-none">Free Cancellation</p>
                    <p className="text-[8px] font-bold text-muted-foreground mt-1 tracking-tight ">Full refund up to 24h before</p>
                </div>
            </div>
        </div>
