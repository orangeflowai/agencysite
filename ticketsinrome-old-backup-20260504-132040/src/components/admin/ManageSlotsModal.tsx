        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-card rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 overflow-y-auto flex-1">
                    ...
                    <div className="space-y-3 mb-8">
                        {slots.length === 0 && (
                            <div className="text-center py-8 bg-muted rounded-xl border border-dashed border-border">
                                <p className="text-muted-foreground font-medium">No slots configured for this date.</p>
                                <p className="text-muted-foreground text-sm">Add one below.</p>
                            </div>
                        )}

                        {slots.map(slot => (
                            <div key={slot.id} className={`flex items-center justify-between p-3 rounded-lg border transition-all ${slot.available_slots === 0 ? 'bg-red-50 border-red-100' : 'bg-card border-gray-100 hover:border-emerald-200 shadow-sm'}`}>
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            {slot.available_slots === 0 && (
                                                <span className="text-[10px] font-bold text-red-600  tracking-wider bg-red-100 px-1.5 py-0.5 rounded">Sold Out</span>
                                            )}
                                        </div>
                                    ...
                                    */}
                                    <button
                                        ...
                                        className={`text-xs font-bold px-3 py-2 rounded-lg transition-colors border ${slot.available_slots > 0
                                            ? 'bg-card border-red-200 text-red-600 hover:bg-red-50'
                                            : 'bg-primary border-emerald-600 text-white hover:opacity-90'
                                            }`}
                                    >
                                        {toggling === slot.id ? '...' : (slot.available_slots > 0 ? 'Close' : 'Open')}
                                    </button>

                                    <button onClick={() => handleDelete(slot.id)} className="text-muted-foreground hover:text-red-600 p-2 hover:bg-gray-100 rounded-lg transition-colors">
                                        <Trash2 size={18} />
                                    </button>
                                </div>
                            </div>
                    ...
                    {/* Add New Form */}
                    <div className="bg-muted p-5 rounded-2xl border border-gray-100">
                        <h4 className="text-xs font-bold text-muted-foreground  tracking-wider mb-4">Add New Slot</h4>
                        <div className="flex items-end gap-3">
                            ...
                            <button
                                onClick={handleAdd}
                                disabled={adding}
                                className="bg-primary hover:opacity-90 text-white p-2.5 rounded-xl transition-transform active:scale-95 shadow-md shadow-emerald-200 h-[42px] w-[42px] flex items-center justify-center mb-[1px]"
                            >
                                {adding ? <Loader2 className="animate-spin w-5 h-5" /> : <Plus size={24} />}
                            </button>
                        </div>
                        <p className="text-[10px] text-muted-foreground mt-2 ml-1">* Leave Price empty to use standard tour price.</p>
                    </div>

                </div>

                {/* Footer */}
                <div className="p-4 bg-muted border-t border-gray-100 flex justify-end shrink-0">
