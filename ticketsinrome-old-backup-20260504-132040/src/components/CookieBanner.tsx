    ...
    return (
        <AnimatePresence>
            {show && (
                ...
                >
                    <div className="max-w-5xl mx-auto bg-card rounded-2xl shadow-2xl border border-gray-100 p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center gap-6">
                        <div className="flex-1">
                            <h3 className="font-bold text-foreground mb-2">{bannerTitle}</h3>
                            <p className="text-sm text-gray-600 leading-relaxed">
                                {bannerText}
                                {' '}{getComplianceText()}
                                <Link href={privacyPolicyLink} className="text-emerald-700 hover:underline font-bold ml-1">
                                    {privacyPolicyText}
                                </Link>.
                            </p>
                        </div>
                        <div className="flex gap-3 shrink-0 w-full md:w-auto">
                            <button
                                onClick={decline}
                                className="flex-1 md:flex-none px-6 py-3 rounded-xl border border-border text-gray-600 font-bold hover:bg-muted transition-colors text-sm"
                            >
                                {declineButtonText}
                            </button>
                            <button
                                onClick={accept}
                                className="flex-1 md:flex-none px-8 py-3 rounded-xl bg-emerald-800 text-white font-bold hover:bg-emerald-900 transition-colors shadow-lg shadow-emerald-900/10 text-sm"
                            >
                                {acceptButtonText}
                            </button>
                        </div>
